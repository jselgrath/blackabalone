...
d1c<-read_excel("./data/Old_Transect.xlsx",sheet="TransectTotals")%>%
  ...
  mutate(species=Species, tx_length=`Transect length (m)`,tx_area=`Transect length (m)`*2,year=year(Date))%>%
...
# cleaning variable names etc
d2a<-d2%>%
  select(site=Site, year=Year, species=Species, ab_size_cm= `Size (cm)` ,
...
# save
write_csv(d3,"./results/tx_old_data_cleaned2.csv")
